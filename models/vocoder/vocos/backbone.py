...
# MIT license.
# Source: https://github.com/jishengpeng/WavTokenizer/blob/main/decoder/models.py
# License included under licenses/LICENSE_wavtokenizer.
...
from .conv import ConvNeXtBlock, ResBlock1, AdaLayerNorm
...
class VocosBackbone(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert num_res % 2 == 0, "num_res should be even number."
        ...
        pos_net = (
            [ResnetBlock(in_channels=block_in,out_channels=block_in,
                        temb_channels=self.temb_ch,dropout=dropout) for _ in range(num_res // 2)]  + 
            [AttnBlock(block_in) for _ in range(num_attn)] +
...
if __name__ == "__main__":
    model = VocosBackbone(1024, 1024, 3072, 24, num_res=8, num_attn=2)
    ...
    with torch.inference_mode():
        macs, params = get_model_complexity_info(model, (1024, 50), print_per_layer_stat=False)
    print(macs, params)