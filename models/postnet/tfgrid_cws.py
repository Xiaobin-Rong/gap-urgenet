...
class TFGridNet(AbsSeparator):
    ...
    def forward(self, x, sr_in=48000):
        ...
        if self.n_imics == 1:
            ...
            assert x.ndim == 3
            std_ = torch.std(x, dim=(1,2), keepdim=True) + 1e-12
            mean_ = torch.mean(x, dim=(1,2), keepdim=True)
            x = (x -mean_) / std_

        if sr_in != 48000:  # e.g., 44100
        ...
        if self.n_imics == 1:
            ...
            input = torch.view_as_real(input)  # (B,C,F,T,2)
            batch = input.permute(0,3,2,1)  # [B, 2, T, F]
        else:
            input = multi_channel_stft(x, nfft, nhop, nfft, torch.hann_window(nfft).to(x.device))  # (B,C,F,T), complex
            input = torch.view_as_real(input)  # (B,C,F,T,2)
            batch = rearrange(input, 'b c f t ri -> b (c ri) t f')
        ...
        if sr_in != 48000:
            output = resample(output, 48000, sr_in, lowpass_filter_width=64)
        output = torch.nn.functional.pad(output, (0, n_samples-output.shape[-1]))
        
        output = output * std_.squeeze(1)
...
if __name__ == "__main__":
    ...
    model = TFGridNet(n_imics=2).cuda()
    ...
    # print("The number of parameters:", params/1e6, "M")
    
    from ptflops import get_model_complexity_info
    
    with torch.inference_mode():
        macs, params = get_model_complexity_info(model, (2, 48000,), print_per_layer_stat=False)
    print(macs, params)

    
    # model = TFGridNet().cuda()
    # x = torch.randn(1, 48000).cuda()
    # y = model(x, sr=48000)
    # print(y.shape)
    
    # z = torch.randn(y.shape).cuda()
    # loss = torch.nn.L1Loss()(y, z)
    # loss.backward()
    
    # for name, param in model.named_parameters():
    #     if not param.grad.is_contiguous():
    #         print(f"Param {name} gradient is not contiguous!")
