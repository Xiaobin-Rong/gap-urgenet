...
class TFGridNet(nn.Module):
    ...
    def forward(self, x, sr=16000):
        ...
        nfft = int(sr * self.fft_len)
        nhop = int(sr * self.hop_len)
        nwin = int(sr * self.win_len)
        input = torch.stft(x, nfft, nhop, nwin, torch.hann_window(nwin).to(x.device), return_complex=True)
    ...
    def from_pretrained(cls, model_ckpt, frozen=True):
        print("[Predictor] Loading from", model_ckpt)
