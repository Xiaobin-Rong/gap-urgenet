...
"""
Inference scripts for long audio (e.g., >20s).
"""
...
MAX_LEN = 8  # Max audio length (s); longer audio is chunked
SEG_LEN = 8  # Segment duration (s)
HOP_SEG_LEN = 4  # Hop size between segments (s)
OLP_SEG_LEN = SEG_LEN - HOP_SEG_LEN  # Overlap duration (s)
...
def inference_file(input_file, output_file, model, **kwargs):
    ...
    Args:
        ...
        output_file (str): Path to save enh_patch audio.
    ...
    audio = torch.from_numpy(audio)[None].to(next(model.parameters()).device)
    
    sr_out = kwargs.get('sr_out', None)
    enable_plc = kwargs.get('enable_plc', True)
    
    seg_len = fs * SEG_LEN
    hop_seg_len = fs * HOP_SEG_LEN
    audio_patches = []
    start = 0
    while start < audio.shape[-1]:
        end = start + seg_len
        if end > audio.shape[-1]:
            if end - audio.shape[-1] < 2 * fs:
                end = audio.shape[-1]
                audio_patches.append(audio[:, start:end])
            else:
                end = audio.shape[-1]
                audio_patches[-1] = torch.cat((audio_patches[-1], audio[:, end_tmp:end]), dim=1)
            break
        audio_patches.append(audio[:, start:end])
        end_tmp = end
        start += hop_seg_len
    ...
    for patch in audio_patches:
        enh_patch = model(patch, sr_in=fs, sr_out=sr_out, enable_plc=enable_plc)
        enh_patches.append(enh_patch)
    
    if len(enh_patches) == 1:
        enhanced = enh_patches[0].cpu().detach().numpy().squeeze()
    
    else:
        enhanced_patches = [enh_patches[0][:, : fs * (SEG_LEN - OLP_SEG_LEN // 2)]]
        for i in range(1, len(enh_patches)-1):
            enhanced_patches.append(enh_patches[i][:, OLP_SEG_LEN // 2 * fs : fs * (SEG_LEN - OLP_SEG_LEN // 2)])
        enhanced_patches.append(enh_patches[-1][:, OLP_SEG_LEN // 2 * fs:])
        enhanced = torch.cat(enhanced_patches, dim=1)
    
    enhanced = enhanced.cpu().detach().numpy().squeeze()
